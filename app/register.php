<?php
	...
	if(isset($_POST['register_individuals'])) {
			...
			$image_one = $_POST['image_one']?$_POST['image_one']:NULL;
			...
			if (isset($_FILES["image_one"]["name"])) {
				$target_file_one = "uploads/".basename($_FILES["image_one"]["name"]);
				$uploadOk_one = 1;
				$imageFileType_one = strtolower(pathinfo($target_file_one,PATHINFO_EXTENSION));
				// Check if image file is a actual image or fake image
			    $check_one = getimagesize($_FILES["image_one"]["tmp_name"]);			
			    if($check_one !== false) {
			    	move_uploaded_file($_FILES["image_one"]["tmp_name"], "uploads/$target_file_one ");
			        $uploadOk_one= 1;
			    } else {
			        echo "File is not an image.";
			        $uploadOk_one = 0;
			...
			try {
					$stmt = $connect->prepare('INSERT INTO room_rental_registrations (fullname, location, email, mobile, alternat_mobile, plot_number, rooms, country, state, city, address, landmark, rent, sale, deposit, description, image, image_one, accommodation, vacant, user_id) 
					VALUES (:fullname, :location, :email, :mobile, :alternat_mobile, :plot_number, :rooms, :country, :state, :city, :address, :landmark, :rent, :sale, :deposit, :description, :image, :image_one , :accommodation, :vacant, :user_id)');
					$stmt->execute(array(
						':fullname' => $fullname,
						':location' => $location,
						':email' => $email,
